...
n = 29
temp = barrier/(boltzmann*n)
temp
#[Out]# 200.0779186647695
(richardson * barrier**2)/(np.exp(n) * n**2 * boltzmann**2)
#[Out]# 1.0182592095082621e-10
saturation_current = 1e-10
richardson = (1e-10 * np.exp(n) * n**2 * boltzmann**2)/(barrier**2)
richardson
#[Out]# 0.0098206821078782106
# Don't forget to scale the current to A m^{-2}
result = {"barrier": barrier_ht, "temp": temp, "richardson": richardson, \
...
richardson = (saturation_current * np.exp(n) * n**2 * boltzmann**2)/(barrier**2)
richardson
